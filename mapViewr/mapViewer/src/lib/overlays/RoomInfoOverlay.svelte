<script>
	...
	/**
	 * @type {{map: import('maplibre-gl').Map | undefined, selectedRoom: SearchRoomResponse | null, selectRoom: Function}}
	 */
	let { map, selectedRoom, selectRoom } = $props();
	...
	function updateMapControlsBottom() {
		...
		/** @type {HTMLElement | null} */
		const bottomRightControl = document.querySelector('.maplibregl-ctrl-bottom-right');
		if (bottomRightControl) {
			bottomRightControl.style.bottom = h + 'px';
		}

		/** @type {HTMLElement | null} */
		const bottomLeftControl = document.querySelector('.maplibregl-ctrl-bottom-left');
	...
	$effect(() => {
		if (typeof ResizeObserver !== 'undefined') {
			resizeObserver = new ResizeObserver(() => updateMapControlsBottom());
			if (overlayElement) resizeObserver.observe(overlayElement);
		}

		if (typeof MutationObserver !== 'undefined') {
			mutationObserver = new MutationObserver(() => updateMapControlsBottom());
...
<div id="room-info-overlay" bind:this={overlayElement} class:visible={selectedRoom}>
	<!-- <RouteOverlay canBeLarge={!large} /> -->
	{#if selectedRoom}
		<div class="room-info-container" in:slide={{ duration: 300 }} out:slide={{ duration: 220 }}>
			<div class="room-info-header" class:has-bottom-border={large}>
				<div>
					<div class="room-info-title">{oldRoomData?.name || 'Room Name'}</div>
					<div class="room-info-building">{oldRoomData?.buildingName || 'Building Name'}</div>
				</div>
				<Controlls
					actions={[
						{
							...
							onClick: () => {
								if (selectedRoom) {
									map?.flyTo({
										center: [selectedRoom.latLng.lng, selectedRoom.latLng.lat],
...
<style>
	...
	#room-info-overlay {
		...
		background: linear-gradient(135deg, rgba(40, 44, 52, 0.97) 0%, rgba(45, 49, 57, 0.95) 100%);
