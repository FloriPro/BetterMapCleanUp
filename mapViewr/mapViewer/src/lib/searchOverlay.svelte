<script>
     ...
     *     selectedMarker: MarkerProposal | null;
     * }}
     */
    let { map, selectRoom, selectedRoom, selectMarker, selectedMarker } =
        $props();
    ...
    $effect(() => {
        if (
            ...
            searchResults[0] instanceof MarkerProposal
        ) {
     ...
     */
    function searchClickHandler(result) {
        ...
        onkeyup={(e) => {
            if (e.key === "Enter") {
                if (searchResults.length > 0) {
                    e.stopPropagation();
                    searchClickHandler(searchResults[0]);
                }
            }
        }}
        role="button"
        ...
        {#if isSearchVisible}
            <div class="type-search-wrapper">
                ...
                {#each availableMapMarkerTypes as type (type.id)}
                    <button
                        type="button"
                        class="type-search-item {type.id ===
                        selectedMarker?.type.id
                            ? 'selected'
                            : ''}"
                        ...
                        onclick={(e) => {
                            if (
                                selectedMarker &&
                                selectedMarker.type.id === type.id
                            ) {
                                clearSearch();
                            } else {
                                e.stopPropagation();
                                searchClickHandler(
                                    new MarkerProposal(type, type.name),
                                );
                            }
                        }}
                    >
        ...
        <div class="search-results">
            <div class="search-results-grid">
                ...
                {#each searchResults.slice(0, 50) as result (result.searchId)}
                    ...
                    <button
                        type="button"
                        class="search-result-item {(result instanceof
                            MarkerProposal &&
                            selectedMarker?.type.id === result.type.id) ||
                        (result instanceof SearchRoomResponse &&
                            selectedRoom?.roomId === result.roomId)
                            ? 'selected'
                            : ''}"
                        onclick={(e) => {
                            e.stopPropagation();
                            searchClickHandler(result);
                        }}
                    >
                        ...
                        {:else if result instanceof SearchRoomResponse}
                            ...
                            <div class="search-result-building">
                                {result.buildingName}
                            </div>
                        {:else}
                            <div class="search-result-name">
