<script>
	...
	/**
	 * @type {{map: import('maplibre-gl').Map | undefined}}
	 */
	let { map } = $props();
	...
	/**
	 * takes maplibre map instance and updates the level select with the current level
	 * @returns {void}
	 */
		...
		{#if availableLevels == null}
			{#each { length: 11 } as _, i (i)}
				<div class="loading-level-button">
					<div class="loading-shimmer" style="animation-delay: {((i * 1) / 3) % 1}s;"></div>
				</div>
			{/each}
		{:else if availableLevels.length > 0}
			{#each availableLevels as level (level)}
				<button
					...
					onclick={() => (currentLevel = level)}>{LEVEL_SHORT[level]}</button
				>
			{/each}
		{:else}
			<button
