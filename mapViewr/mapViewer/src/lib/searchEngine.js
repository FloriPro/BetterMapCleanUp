...
const searchHandler = new (class SearchHandler {
	 ...
	 * 
	 * @param {string} value 
	 * @returns {Array<MarkerProposal>}
	 */
	checkMarkerProposals(value) {
		let out = [];
		for (let type of availableMapMarkerTypes) {
			let simmilarity = Math.max(
				this.similarAnywhere(this.normalizeText(type.name), this.normalizeText(value)),
				this.similarAnywhere(this.normalizeText(type.selector), this.normalizeText(value))
			);
			if (simmilarity > 0.7) {
	...
	search(value) {
		...
		/**
		 * @type {Array<{type: string, room?: string, building?: string, subtr?: boolean}>}
		 */
		let checks = [
			{
				"type": "room",
		...
		const r = /\b([a-z]{0,2}\s?u?\d{2,5}[a-z]?)\b/ig;
		let maybeRoomNumberIt = normalizedValue.matchAll(r);
		for (const maybeRoomNumber of maybeRoomNumberIt) {
			...
			let buildingName = normalizedValue.replace(maybeRoomNumberValue, "").trim();
			if (buildingName && buildingName.length >= 2) {
				checks.push({
					"type": "buildingAndRoom",
					"building": this.removeSpace(buildingName),
					"room": this.removeSpace(maybeRoomNumberValue)
				});
			} else {
				checks.push({
					"type": "room",
		...
		for (const [buildingId, building] of Object.entries(data.part)) {//this.main.buildingData.part
			...
			for (const part of Object.values(building.parts || {})) {
				for (const room of Object.values(part.rooms || {})) {
					...
					for (const check of normalizedChecks) {
						...
						if (check.type === "room") {
							if (!check.normalizedRoom) continue;
							...
							if (!check.normalizedRoom || !check.normalizedBuilding) continue;
							similarity = this.similar(normalizedRoomName, check.normalizedRoom);
							similarity += this.similarStart(normalizedBuildingName, check.normalizedBuilding);
						}
		...
		for (let i = 0; i < filteredRooms.length; i++) {
			...
			if (result instanceof SearchRoomResponse) {
				idString += "srr_" + result.buildingName + result.buildingId + result.roomId + result.level;
			} else if (result instanceof MarkerProposal) {
				idString += "mp_" + result.type.id;
			}
			// if the idString already exists, remove this result, otherwise add the idString to existing
